...
class KinectService():
    ...
    def reduce_start_times(self):
        """ Finds the earliest starting stream (body, depth, colour etc) and reduces all streams to starting points relative to that """
        sub = min(self._streams, key=lambda x: x.start_time()).start_time()
    ...
    def write_video(self):
        """ Closes the connection to the video writer """
        self._video.release()

    ### Main Loop
        
    def listen(self, getBodies=True, getAudio=False, getVideo=False, getDepth=False, Clicking=False):
        """ Main application loop """

        # TODO -> Sort out adding streams

        self._streams.append(self._body_stream)
        self._streams.append(self._video_stream)
        ...
        while not self._done:
            ...
            if getVideo:
                ...
                if self._video_stream.has_new_frame( rgb_frame ):
                    ...
                    self._video_stream.body.append( (self._body_stream.last_frame_index(), self._body_stream.end_time()) )

                    self.update_timings()                               # Update timings

            #print self._body_stream.has_new_frame( body_frame ),
            #print self._video_stream.has_new_frame( rgb_frame )
    ...
    def store_media_times(self, body, audio, video):
        ...
        if audio:
            ...
            self.write_stream_timestamps(AUDIO_TIME_TABLE, self._audio_stream)
        if video:
